...
description: Prepares the implementation environment for an adventure task. Creates git branches, validates evaluations, and injects adventure context into the task file.
...
You receive a task file path for a task with an `adventure_id`. Prepare the implementation environment: set up git branch/worktree, validate evaluations, and inject adventure context into the task.
...
3. If significant codebase drift is detected (files deleted, renamed, or substantially changed), log a warning in the task log but continue
...
1. Create a minimal test file stub at the referenced path
2. Add the file to the task's `files` list
3. Log the scaffolding in the task log

## Step 5: Complete

1. Append to `## Log`: `- [{timestamp}] adventure-preparer: Prepared environment -- {branch info}, {N} conditions injected, {N} test stubs created`
2. Set frontmatter `status: ready`

## Rules

- Only create git branches, not worktrees (worktree support deferred)
- Do not modify project source code beyond test stubs
- If git operations fail, log the error but still inject context and set ready
